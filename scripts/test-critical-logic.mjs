...
const gasUtils = fs.readFileSync(new URL('../Sheet WEB/DM_Len De/GAS_4_Utils.gs', import.meta.url), 'utf8');
...
const gasApi = fs.readFileSync(new URL('../Sheet WEB/DM_Len De/GAS_5_Api.gs', import.meta.url), 'utf8');
const gasContentAdmin = fs.readFileSync(new URL('../Sheet WEB/DM_Bao Mat/GAS_6_Content_Admin.gs', import.meta.url), 'utf8');
const gasAnswerKeySystem = fs.readFileSync(new URL('../Sheet WEB/DM_Bao Mat/GAS_7_Answer_Key_System.gs', import.meta.url), 'utf8');
const gasQuestionOverrides = fs.readFileSync(new URL('../Sheet WEB/DM_Bao Mat/GAS_8_Question_Overrides.gs', import.meta.url), 'utf8');
const gasMenu = fs.readFileSync(new URL('../Sheet WEB/DM_Len De/GAS_1_Menu.gs', import.meta.url), 'utf8');
...
assert.equal(gasSync.includes("'Giá Bán', 'Ghi Chú Giá'"), true, 'Standalone document catalog must own its price and price note columns');
...
assert.equal(fs.existsSync(new URL('../Sheet WEB/DM_Bao Mat/GAS_User_Access_Admin.html', import.meta.url)), false, 'Unused access admin HTML file must stay deleted');
...
assert.equal(gasMenu.includes('sheet.insertImage(QUICK_ACTION_BUTTON_URL_'), true, 'Transparent Sheet drawings must be replaced by a visible action image');

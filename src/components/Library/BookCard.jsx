...
export default function BookCard({ book, onAskAi, onUnlock, isUnlocked }) {
  ...
  return (
    <motion.div
      ...
      className="group relative rounded-2xl sm:rounded-3xl bg-white dark:bg-[#0b1120] backdrop-blur-xl border border-slate-200/80 dark:border-white/10 p-3 sm:p-5 shadow-sm hover:shadow-2xl dark:hover:shadow-teal-500/10 transition-all duration-300 flex flex-col justify-between overflow-hidden"
    >
      {/* Glow Hover Ambient */}
      ...
      <div>
        {/* ================================================================= */}
        {/* 1. COVER MỜ DẦN BẰNG CSS MASK IMAGE                               */}
        {/* ================================================================= */}
        <div 
          className="-mx-3 -mt-3 sm:-mx-5 sm:-mt-5 h-32 sm:h-56 relative overflow-hidden bg-transparent pointer-events-none [mask-image:linear-gradient(to_bottom,black_20%,transparent_98%)] [-webkit-mask-image:linear-gradient(to_bottom,black_20%,transparent_98%)]"
        >
          {directCover ? (
            <img
              src={directCover}
          ...
          <div className={`${directCover ? 'hidden' : 'flex'} w-full h-full bg-gradient-to-br ${defaultCoverGradient} flex items-center justify-center text-white/90`}>
            <div className="w-10 h-10 sm:w-16 sm:h-16 rounded-xl sm:rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center border border-white/20 shadow-inner">
        ...
        {/* 2. THÔNG TIN SÁCH                                                 */}
        {/* ================================================================= */}
        <div className="relative z-10 -mt-6 sm:-mt-8 space-y-1 sm:space-y-2">
          {book.department && (
            <span className="hidden sm:inline text-[10px] sm:text-[11px] font-extrabold text-teal-600 dark:text-teal-400 uppercase tracking-wide">
              {book.department}
            </span>
          )}

          <h3 className="font-bold sm:font-extrabold text-xs sm:text-base text-slate-900 dark:text-white leading-snug line-clamp-2 group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors">
            {book.title}
          </h3>

          <div className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 space-y-0.5 sm:space-y-1 pt-0.5">
            {book.author && (
              <div className="flex items-center space-x-1 sm:space-x-1.5">
                <User className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0 text-slate-400" />
      ...
      <div className="mt-3 sm:mt-5 pt-2 sm:pt-3 border-t border-slate-100 dark:border-white/10 flex items-center space-x-1.5 sm:space-x-2 relative z-10">
        ...
        <button
          ...
          className="p-1.5 sm:p-2.5 rounded-lg sm:rounded-xl bg-cyan-50 dark:bg-cyan-950/40 hover:bg-cyan-100 dark:hover:bg-cyan-900/50 border border-cyan-200/80 dark:border-cyan-800/40 text-cyan-600 dark:text-cyan-400 transition-all group/ai shrink-0"
          title="Hỏi AI về sách này"
        >
          <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 group-hover/ai:rotate-12 transition-transform" />
        </button>
      </div>
    </motion.div>
  );
